...
function [x,obj,incr]=PD_ChambollePock_Covid(Z, objective, op, prox, opts)


    % Inputs:  - Z: observed corrupted data
    ...
    while (incrc > opts.prec) && (i < opts.iter)
        ...
        x   = prox.fidelity(x0 - tau * op.adjoint(y),Z,tau);
        ...
        obj(i) = objective.fidelity(x,Z) +  objective.regularization(op.direct(x),1);
